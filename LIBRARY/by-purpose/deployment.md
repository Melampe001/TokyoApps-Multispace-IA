# Deployment

Total: 20 archivos

- [BUILD_AGENT_REPORT.md](flutter_app/BUILD_AGENT_REPORT.md) - BUILD_AGENT Implementation Report
- [Makefile](Makefile) - Makefile - build
- [agent_deploy.dart](flutter_app/lib/agents/agent_deploy.dart) - agent_deploy.dart - source
- [agents-deployment.yaml](infrastructure/k8s/agents-deployment.yaml) - tokyo-ia-agents
- [build.go](internal/orchestrator/agents/build.go) - build.go - source
- [build.gradle](flutter_app/android/build.gradle) - build.gradle - build
- [build.gradle](flutter_app/android/app/build.gradle) - build.gradle - build
- [build.md](LIBRARY/by-category/build.md) - Build
- [build_agent.sh](flutter_app/build_agent.sh) - !/bin/bash
- [deploy-vercel.yml](.github/workflows/deploy-vercel.yml) - Deploy Web to Vercel
- [deploy.sh](scripts/deploy.sh) - !/bin/bash
- [deploy_with_agents.sh](scripts/deploy_with_agents.sh) - !/bin/bash
- [deployer.go](lib/generator/deployer.go) - deployer.go - source
- [deployment.md](LIBRARY/by-purpose/deployment.md) - Deployment
- [flutter-build-agent.yml](.github/workflows/flutter-build-agent.yml) - Flutter Build Agent
- [orchestrator-deployment.yaml](infrastructure/k8s/orchestrator-deployment.yaml) - tokyo-ia-orchestrator
- [preview-deploy.yml](.github/workflows/preview-deploy.yml) - 🚀 Preview Deployment
- [release-drafter.yml](.github/workflows/release-drafter.yml) - 📝 Release Drafter
- [release.yml](.github/workflows/release.yml) - 📦 Release
- [settings.gradle](flutter_app/android/settings.gradle) - settings.gradle - build
