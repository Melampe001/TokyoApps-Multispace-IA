# Automation

Total: 32 archivos

- [.codecov.yml](.codecov.yml) - .codecov.yml - workflow
- [.golangci.yml](.golangci.yml) - .golangci.yml - workflow
- [.pre-commit-config.yaml](.pre-commit-config.yaml) - .pre-commit-config.yaml - workflow
- [Agentes y bots](Agentes y bots) - Agentes y bots - other
- [IMPLEMENTATION_SUMMARY_BOTS.md](IMPLEMENTATION_SUMMARY_BOTS.md) - Implementation Summary: Automated PR Bots
- [README.md](.github/workflows/README.md) - GitHub Workflows Documentation
- [README.md](.github/workflows/bots/README.md) - 🤖 Tokyo-IA Automated Bots Documentation
- [README.md](templates/bot/README.md) - Bot Template
- [SLACK_BOT_SETUP.md](docs/integrations/SLACK_BOT_SETUP.md) - Slack Bot Setup Guide
- [WorkflowMonitor.tsx](admin/src/components/WorkflowMonitor.tsx) - WorkflowMonitor.tsx - source
- [__init__.py](lib/orchestrator/__init__.py) - Tokyo-IA Orchestrator

Multi-agent workflow orchestration system.
- [analysis_options.yaml](flutter_app/analysis_options.yaml) - analysis_options.yaml - workflow
- [auto_assign.yml](.github/auto_assign.yml) - auto_assign.yml - workflow
- [automation.md](LIBRARY/by-purpose/automation.md) - Automation
- [bot-backend-performance.yml](.github/workflows/bot-backend-performance.yml) - Bot #2 - Backend Performance Monitor
- [bot-backend-quality.yml](.github/workflows/bot-backend-quality.yml) - Bot #1 - Backend Code Quality Guardian
- [bot-coordinator.yml](.github/workflows/bot-coordinator.yml) - Bot Coordinator - Weekly Report
- [bot-frontend-build.yml](.github/workflows/bot-frontend-build.yml) - Bot #4 - Frontend Build & Asset Optimizer
- [bot-frontend-ux.yml](.github/workflows/bot-frontend-ux.yml) - Bot #3 - Frontend UI/UX Compliance Bot
- [bots-overview.md](docs/automation/bots-overview.md) - Automation Bots Overview
- [dependabot.yml](.github/dependabot.yml) - dependabot.yml - workflow
- [deployment.yaml](config/agents/deployment.yaml) - deployment.yaml - workflow
- [integration_mappings.yaml](config/agents/integration_mappings.yaml) - integration_mappings.yaml - workflow
- [labeler.yml](.github/labeler.yml) - labeler.yml - workflow
- [pr-auto-review.yml](.github/workflows/pr-auto-review.yml) - 🤖 PR Auto-Review Bot
- [release-drafter.yml](.github/release-drafter.yml) - release-drafter.yml - workflow
- [shell-automation.md](docs/elara/agents/shell-automation.md) - 🔧 Shell Agent - Automation Specialist
- [slack-bot.yml](.github/workflows/slack-bot.yml) - Slack Bot
- [slack_bot.py](.github/workflows/scripts/slack_bot.py) - !/usr/bin/env python3
- [workflow.md](LIBRARY/by-category/workflow.md) - Workflow
- [workflows.py](lib/orchestrator/workflows.py) - !/usr/bin/env python3
- [workflows.py](lib/agents/workflows.py) - Multi-agent workflows for Tokyo-IA.

This module defines workflows that coordinate multiple agents:

