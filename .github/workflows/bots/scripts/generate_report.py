...
class BotReportGenerator:
    ...
    def aggregate_metrics(self) -> Dict:
        ...
        if self.reports["backend_quality"]:
            ...
            aggregate["backend_quality"]["issues"] = report.get("summary", {}).get(
                "total_issues", 0
            )
        ...
        # Calculate totals
        self.summary["total_prs_analyzed"] = sum(bot["analyzed"] for bot in aggregate.values())
        self.summary["total_issues_found"] = sum(bot.get("issues", 0) for bot in aggregate.values())
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="bot-coordinator-report.json", help="Output file"
    )
