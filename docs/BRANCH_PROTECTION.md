...
## Branch Cleanup and Retention Policy

### Automatic Branch Cleanup

To maintain a clean repository, we have implemented an automatic branch cleanup system that removes obsolete merged branches.

#### Policy Overview

- **Retention Period**: 14 days after merge to `Main`
- **Execution**: Weekly (Sundays at 00:00 UTC)
- **Method**: Automatic via GitHub Actions workflow
...
Prompt        # Prompt configuration branch  
main          # Main branch (lowercase)
develop       # Development branch
mela          # Special project branch
```

#### Protected Patterns

Branches matching these patterns are automatically protected:

```
feature/*     # Feature branches in development
hotfix/*      # Urgent patches
release/*     # Release branches
```

#### How to Exclude a Branch

If you need to prevent a specific branch from being deleted after merge:

**Option 1: Add to Exclusion List**

Edit `.github/branch-cleanup-exclude.txt`:
...
After each automatic cleanup, an issue is created with:
- Number of branches deleted
- List of deleted branches
- Statistics and summary

Look for issues labeled `cleanup` and `automated`.

For complete documentation, see [BRANCH_CLEANUP.md](./BRANCH_CLEANUP.md).
...
- [Branch Cleanup System](./BRANCH_CLEANUP.md)
